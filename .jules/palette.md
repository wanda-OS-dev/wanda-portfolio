...
## 2024-04-26 - Parity Between Hover and Keyboard Focus
**Learning:** To ensure visual feedback parity for keyboard users on complex interactive elements (like cards), we must pair Tailwind `group-hover:*` utility classes with their equivalent `group-focus-visible:*` classes. This ensures all visual cues (like lines expanding or opacities changing) occur predictably on tab focus.
**Action:** Always pair `group-hover` with `group-focus-visible` when building complex card-based `Link` components.
