...
## 2026-04-10 - Animated Copied Feedback
**Learning:** When implementing utility actions like 'Copy to clipboard' that do not trigger native browser alerts, do not rely solely on subtle icon changes or `title` attributes. Instead, provide explicit, temporary text-based visual feedback (e.g., an animated 'Copied!' span) adjacent to the trigger to ensure unambiguous confirmation. When using components like Framer Motion's `AnimatePresence` to display this explicit visual text feedback alongside actions that already utilize `aria-live` screen reader announcements, consistently apply `aria-hidden="true"` to the visual element to prevent duplicate or noisy announcements.
**Action:** Consistently add temporary visual text feedback elements (e.g., `<AnimatePresence>` wrapped spans) to copy-to-clipboard buttons, ensuring `aria-hidden="true"` is applied when a separate `aria-live` region handles the screen reader announcement.
