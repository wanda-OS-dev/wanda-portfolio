...
## 2025-06-25 - Making WebGL and 3D Canvas Elements Accessible
**Learning:** WebGL canvases (like those generated by `@react-three/fiber`) are opaque to screen readers by default. When a canvas is used purely for decorative backgrounds, it should be explicitly hidden. However, when it contains interactive or informative content (like a 3D graph or data visualization), it must provide an accessible alternative description.
**Action:** Always wrap informative WebGL `<Canvas>` components in a container `div` with `role="img"` and a descriptive `aria-label` to ensure screen readers announce its meaning. For purely decorative WebGL backgrounds, apply `aria-hidden="true"` to the container to safely remove it from the accessibility tree.
