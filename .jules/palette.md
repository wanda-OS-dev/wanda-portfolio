...
## 2025-03-31 - Improved skip-to-content focus routing
**Learning:** React/Next.js Single Page Applications often trap focus when using hash links (`href="#main-content"`) if the target container doesn't have a specific `tabIndex`. Without `tabIndex={-1}`, the browser struggles to properly transfer keyboard focus context into the `<main>` element, potentially forcing the user to tab through the navigation again.
**Action:** When adding skip-to-content links targeting structural containers (like `<main>`), always ensure the target container includes `tabIndex={-1}` and `outline-none` (or equivalent) to receive programmatic focus cleanly without creating a lingering visual outline ring.
