...
## 2026-04-05 - WebGL/Three.js Screen Reader Accessibility
**Learning:** When using Three.js or WebGL canvases, screen readers may ignore or get confused by the interactive or decorative elements if they are not explicitly labelled. Purely decorative scenes should be explicitly hidden using `aria-hidden="true"`, whereas informative and interactive 3D visualizations require a `role="img"` and a descriptive `aria-label` on their wrapper to ensure users relying on screen readers receive proper context.
**Action:** Always add `role="img"` and `aria-label` to informative WebGL wrappers, and `aria-hidden="true"` to purely decorative WebGL wrappers.
