...
## 2026-04-01 - Enhance Form Usability with AutoComplete Attributes
**Learning:** Standard HTML form fields requesting common personal information (such as name or email) can create unnecessary friction for users if they must manually type the information every time. Some users, particularly those using assistive technologies or on mobile devices, rely heavily on browser autofill to bypass cognitive and physical barriers to data entry.
**Action:** To reduce user friction and improve accessibility, always include appropriate `autoComplete` attributes (e.g., `autoComplete="name"`, `autoComplete="email"`) on standard HTML form fields.
