...
export function Nav() {
  ...
  // Pre-calculate active states to prevent redundant string checks during scroll re-renders
  const activeStates = useMemo(() => {
    const states = new Map<string, boolean>();
    for (const link of links) {
      states.set(link.href, pathname === link.href || pathname.startsWith(link.href + '/'));
    }
    return states;
  }, [pathname]);
      ...
      >
        ...
        <nav className="hidden md:flex items-center gap-8">
          {links.map(({ href, label }) => {
            const isActive = activeStates.get(href) ?? false;
            return (
              <Link
                key={href}
                href={href}
